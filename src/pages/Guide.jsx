...
const Section = ({ title, children }) => (
  <div className="mb-8">
    <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200 mb-3 pb-2 border-b border-slate-200 dark:border-slate-700">{title}</h2>
    {children}
  </div>
);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-900">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-2">
              <Link to={createPageUrl('Settings')}>
                <button className={`flex items-center gap-1.5 px-3 py-1 rounded-lg border text-sm font-medium transition-colors ${isSettingsActive ? 'bg-blue-50/60 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-700' : 'bg-blue-50/60 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-blue-100 dark:border-slate-700 hover:bg-blue-100/60 dark:hover:bg-slate-700'}`}>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border-[1.5px] border-[#DCE8F6] dark:border-slate-700 shadow-sm p-8">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">User Guide</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mb-8">Everything you need to know to get started with Literature Tracker.</p>

          <p className="mb-8 leading-relaxed text-slate-700 dark:text-slate-300">
          ...
          <Section title="Tabs">
            <ul className="space-y-4 text-sm text-slate-700 dark:text-slate-300">
              <li>
                <span className="font-semibold text-slate-800 dark:text-slate-200">Feed</span> – Live RSS articles from your followed journals, sorted by date. Click the title or arrow icon to open the article. Use the bookmark icon to save an article.
              </li>
              <li>
                <span className="font-semibold text-slate-800 dark:text-slate-200">Saved</span> – All your bookmarked articles. Export them as RIS, CSV, or plain text. Set up <strong>Auto-Save Rules</strong> to automatically bookmark articles matching specific keywords or authors.
              </li>
            </ul>
          </Section>

          <Section title="Filters & Persistent Filters">
            <p className="text-sm text-slate-700 dark:text-slate-300 mb-3">The Feed tab has a filter bar where you can:</p>
            <ul className="space-y-2 text-sm text-slate-700 dark:text-slate-300 list-disc list-inside mb-4">
          ...
          <Section title="Tips">
            <ul className="space-y-2 text-sm text-slate-700 dark:text-slate-300 list-disc list-inside">
              <li>Articles you've scrolled past appear in <span className="text-slate-500 dark:text-slate-400 font-medium">gray</span>; unread ones are <span className="text-blue-600 dark:text-blue-400 font-medium">blue</span></li>

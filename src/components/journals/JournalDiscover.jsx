    ...
    <div className="space-y-3">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-8 py-2 text-sm rounded-lg border border-border bg-muted focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent"
        />
        {keyword && (
          <button onClick={() => setKeyword('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-slate-600">
      ...
      <div className="flex items-center gap-2 px-1">
        {isKeywordMode
          ? <><Search className="w-3.5 h-3.5 text-muted-foreground" /><span className="text-xs text-muted-foreground">Results for "<strong>{keyword}</strong>"</span></>
          : <><Sparkles className="w-3.5 h-3.5 text-purple-500" /><span className="text-xs text-muted-foreground">Suggested based on your followed journals</span></>
        }
      </div>
          ...
          {displayList.map(j => {
            ...
            return (
              ...
              >
                ...
                {j.category && (
                  <span className="flex-shrink-0 text-[10px] px-1.5 py-0.5 rounded-full bg-slate-100 text-muted-foreground hidden sm:block">
                    {j.category}
                  </span>
                )}
              </button>

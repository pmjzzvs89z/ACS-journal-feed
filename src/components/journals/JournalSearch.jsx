    ...
    <div className="space-y-2">
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-8 rounded-lg border border-slate-200 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
      ...
      {query.trim() && (
        results.length === 0 ? (
          <p className="text-sm text-slate-400 text-center py-6">No journals found for "<strong>{query}</strong>".</p>
        ) : (
          <div className="space-y-0.5">
            <p className="text-xs text-slate-400 px-1 pb-1">{results.length} journal{results.length !== 1 ? 's' : ''} found</p>
            {results.map(j => {
